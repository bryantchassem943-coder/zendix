    ...
    await mailTransporter.sendMail({
      ...
      html: `
        <div style="font-family:monospace;background:#04050d;color:#e2e8f0;padding:32px;border-radius:12px;">
          ...
          <p style="color:#64748b;margin-bottom:24px;">${agencyName || 'RELAY'} — Alerte lead chaud</p>
...
// ─── NOTIFICATION TELEGRAM (alternative fiable a l'email/WhatsApp pour les alertes internes) ──
function sendTelegramMessage(chatId, text) {
  return new Promise(function(resolve, reject) {
    if (!TELEGRAM_BOT_TOKEN) { console.log('Telegram IGNORE: TELEGRAM_BOT_TOKEN non configure sur le serveur.'); return resolve(null); }
    if (!chatId) { console.log('Telegram IGNORE: aucun chat_id Telegram configure pour ce client (onglet Canaux).'); return resolve(null); }
    ...
    var req = https.request(options, function(res) {
      ...
      res.on('end', function() {
        try {
          ...
          if (parsed.ok) { console.log('Telegram envoye a:', chatId); resolve(parsed); }
          else { console.error('Telegram erreur:', parsed.description); resolve(null); }
        } catch(e) { console.error('Telegram parse erreur:', data.slice(0,200)); resolve(null); }
      });
    });
    req.on('error', function(e) { console.error('Telegram reseau erreur:', e.message); resolve(null); });
    req.write(body);
    req.end();
  });
}

function sendHotLeadTelegram(chatId, lead, agencyName) {
  var text =
    '🔥 <b>Lead chaud detecte</b>\n' +
    (agencyName ? agencyName + '\n\n' : '\n') +
    '<b>Nom :</b> ' + (lead.name || '—') + '\n' +
    '<b>Canal :</b> ' + (lead.channel || 'whatsapp').toUpperCase() + '\n' +
    '<b>Email :</b> ' + (lead.email || '—') + '\n' +
    '<b>Budget :</b> ' + (lead.budget || '—') + '\n' +
    '<b>Score :</b> ' + (lead.score || 0) + '/10\n' +
    '<b>Telephone :</b> ' + (lead.phone || '—');
  return sendTelegramMessage(chatId, text);
}

function sendRdvTelegram(chatId, lead, date, time, durationMinutes, location, notes) {
  var text =
    '📅 <b>RDV confirme</b>\n\n' +
    '<b>Prospect :</b> ' + (lead.name || 'Inconnu') + '\n' +
    '<b>Date :</b> ' + date + ' a ' + time + '\n' +
    '<b>Duree :</b> ' + durationMinutes + ' min' +
    (location ? '\n<b>Lieu :</b> ' + location : '') +
    (notes ? '\n<b>Notes :</b> ' + notes : '');
  return sendTelegramMessage(chatId, text);
}
...
function generateICS(opts) {
  var uid = opts.uid || (uuidv4() + '@relayapp');
  var now = new Date();
...
// ─── CALCUL DES CRENEAUX REELLEMENT DISPONIBLES (sans dependance Google/Outlook) ──
...
async function computeAvailableSlots(clientId, agencyConfig, count) {
  ...
  var startMin = timeStrToMinutes(agencyConfig && agencyConfig.rdv_start);
  var endMin = timeStrToMinutes(agencyConfig && agencyConfig.rdv_end || '17:00');
  var slotMin = (agencyConfig && agencyConfig.rdv_slot_minutes) || 30;

  // Recupere les creneaux deja pris (futurs) pour ce client, pour eviter tout conflit
  var bookedSet = {};
  try {
    var r = await supabase.from('leads').select('rdv_at').eq('client_id', clientId).not('rdv_at', 'is', null).gt('rdv_at', new Date().toISOString());
    (r.data || []).forEach(function(row) {
      if (row.rdv_at) bookedSet[new Date(row.rdv_at).toISOString().slice(0, 16)] = true; // precision a la minute
    });
  } catch(e) { console.error('computeAvailableSlots erreur lecture leads:', e.message); }

  var slots = [];
  var d = new Date();
  d.setDate(d.getDate() + 1); // on ne propose jamais le jour meme
  d.setHours(0, 0, 0, 0);

  for (var dayOffset = 0; dayOffset < 21 && slots.length < count; dayOffset++) {
    var candidateDay = new Date(d);
    candidateDay.setDate(d.getDate() + dayOffset);
    var isoDay = candidateDay.getDay() === 0 ? '7' : String(candidateDay.getDay());
    if (days.indexOf(isoDay) === -1) continue;

    for (var m = startMin; m < endMin && slots.length < count; m += slotMin) {
      var slotDate = new Date(candidateDay);
      slotDate.setHours(0, 0, 0, 0);
      slotDate.setMinutes(m);
      var key = slotDate.toISOString().slice(0, 16);
      if (bookedSet[key]) continue; // deja pris, on saute
      slots.push({
        date: slotDate.toISOString().slice(0, 10),
        time: minutesToTimeStr(m),
        label: JOURS_FR[candidateDay.getDay()] + ' ' + candidateDay.getDate() + '/' + (candidateDay.getMonth() + 1) + ' a ' + minutesToTimeStr(m).replace(':', 'h')
      });
    }
  }
  return slots;
...
async function tryAutoConfirmRdv(session, leadInfo, availableSlots, agencyConfig, leadId) {
  if (session.rdvAutoBooked) return null; // deja reserve pour cette session, on ne rejoue pas
  if (!agencyConfig || !agencyConfig.rdv_auto_confirm) return null;
  if (!leadInfo.creneau_confirme || !leadId) return null;
  if (!availableSlots || !availableSlots.length) return null;

  var wanted = String(leadInfo.creneau_confirme).trim().toLowerCase();
  var slot = availableSlots.find(function(s) { return s.label.trim().toLowerCase() === wanted; });
  if (!slot) {
    // Correspondance exacte non trouvee : on tente une correspondance partielle avant d'abandonner
    slot = availableSlots.find(function(s) {
      var l = s.label.trim().toLowerCase();
      return wanted.indexOf(l) !== -1 || l.indexOf(wanted) !== -1;
    });
  }
  if (!slot) { console.log('Auto-RDV IGNORE — creneau_confirme ne correspond a aucun creneau propose:', leadInfo.creneau_confirme); return null; }

  session.rdvAutoBooked = true;
  try {
    var durationMinutes = (agencyConfig.rdv_slot_minutes) || 30;
    var startAt = new Date(slot.date + 'T' + slot.time + ':00');
    var result = await finalizeRdv(leadId, startAt, durationMinutes, 'RDV', '', 'Confirme automatiquement par l\'agent IA.');
    console.log('Auto-RDV confirme pour lead', leadId, '—', result.dateStr, result.timeStr);
    return result;
  } catch(e) {
    console.error('Auto-RDV erreur:', e.message);
    return null;
  }
}
...
function buildSystemPrompt(config, availableSlots) {
  ...
  if (config && config.agency_name) {
    ...
    base += 'Tu es RELAY, ton agent de conversion.\n\n';
  }
  ...
  base += '[DATA]{"lead_info":{"nom":null,"email":null,"entreprise":null,"budget":null,"objectif":null,"moyen_contact":null,"creneau_souhaite":null,"creneau_confirme":null},"lead_score":0,"score_reason":"","info_collected":false}[/DATA]. ';
...
function uploadAudioToMeta(audioBuffer, phoneNumberId, waToken) {
  return new Promise(function(resolve, reject) {
    var boundary = '----RelayBoundary' + Date.now();
...
function uploadMediaToMeta(buffer, mimeType, filename, phoneNumberId, waToken) {
  return new Promise(function(resolve, reject) {
    var boundary = '----RelayMediaBoundary' + Date.now();
    var part1 = Buffer.from(
      '--' + boundary + '\r\nContent-Disposition: form-data; name="messaging_product"\r\n\r\nwhatsapp\r\n' +
...
async function transcribeAudioBuffer(audioBuffer) {
  var boundary = '----RelayWhisper' + Date.now();
  var part1 = Buffer.from(
...
async function saveToSupabase(leadInfo, score, reason, sessionId, clientId, channel) {
  ...
  if (existing.data) {
    var updatePayload = {
      ...
      desired_slot: leadInfo.creneau_souhaite || undefined,
      score: score,
    ...
    var payload = {
      ...
      desired_slot: leadInfo.creneau_souhaite || null,
      score: score,
      status: score >= 9 ? 'hot' : score >= 7 ? 'warm' : 'new',
      pipeline_stage: score >= 9 ? 'CLOSING' : score >= 7 ? 'DEMO_SCHEDULED' : 'PROSPECT',
...
async function processIncomingText(params) {
  ...
  if (session.turns >= 1) {
    ...
    if (!session.notifSent && session.leadScore >= 7) {
      session.notifSent = true;
      if (agencyConfig && agencyConfig.notification_email) {
        ...
        console.log('Email hot lead NON declenche pour', from, '— aucun notification_email configure.');
      }
      if (agencyConfig && agencyConfig.telegram_chat_id) {
        await sendHotLeadTelegram(agencyConfig.telegram_chat_id, savedLead, agencyConfig.agency_name);
      } else {
        console.log('Telegram hot lead NON declenche pour', from, '— aucun telegram_chat_id configure.');
      }
    }

    var autoRdv = await tryAutoConfirmRdv(session, session.leadInfo, availableSlots, agencyConfig, savedLead && savedLead.id);
    if (autoRdv) {
      var confirmMsg = '✅ C\'est confirmé ! Votre rendez-vous est réservé pour le ' + autoRdv.dateStr + ' à ' + autoRdv.timeStr + '. Vous recevrez les détails par le moyen de contact indiqué.';
      await saveMessage(from, clientId, channel, 'assistant', confirmMsg);
      await sendReplyByChannel(channel, from, confirmMsg, sendCtx);
    }
  }
  ...
  if (session.leadInfo.email) {
    ...
    if (!session.notifSent && session.leadScore >= 7) {
      ...
      if (agencyConfig && agencyConfig.notification_email) {
        ...
        console.log('Email hot lead NON declenche pour', sessionId, '— aucun notification_email configure.');
      }
      if (agencyConfig && agencyConfig.telegram_chat_id) {
        await sendHotLeadTelegram(agencyConfig.telegram_chat_id, savedLead, agencyConfig.agency_name);
      } else {
        console.log('Telegram hot lead NON declenche pour', sessionId, '— aucun telegram_chat_id configure.');
      }
    }
    ...
    if (autoRdv) {
      var confirmMsg = '✅ C\'est confirmé ! Votre rendez-vous est réservé pour le ' + autoRdv.dateStr + ' à ' + autoRdv.timeStr + '. Vous recevrez les détails par le moyen de contact indiqué.';
      await saveMessage(sessionId, clientId, 'web', 'assistant', confirmMsg);
      clean = clean + '\n\n' + confirmMsg;
    }
  }
...
// ─── CONFIRMATION D'UN RDV (reutilisable : bouton manuel dashboard OU mode automatique) ──
async function finalizeRdv(leadId, startAt, durationMinutes, title, location, notes) {
  var leadR = await supabase.from('leads').select('*').eq('id', leadId).single();
  if (leadR.error || !leadR.data) throw new Error('Lead introuvable');
  var lead = leadR.data;

  var agencyConfigForRdv = await getAgencyConfig(lead.client_id);
  if (!location && agencyConfigForRdv && agencyConfigForRdv.default_location) {
    location = agencyConfigForRdv.default_location;
  }

  var uid = uuidv4() + '@relayapp';
  ...
  var dateStr = startAt.toISOString().slice(0, 10);
  var timeStr = minutesToTimeStr(startAt.getHours() * 60 + startAt.getMinutes());

  await saveNotification(lead.client_id, leadId, 'rdv_scheduled',
    'RDV planifié — ' + (lead.name || 'Prospect'),
    'Le ' + dateStr + ' à ' + timeStr);

  if (mailTransporter && agencyConfigForRdv && agencyConfigForRdv.notification_email) {
    try {
      await mailTransporter.sendMail({
        from: '"RELAY" <' + GMAIL_USER + '>',
        to: agencyConfigForRdv.notification_email,
        subject: '📅 RDV confirmé — ' + (lead.name || 'Prospect') + ' (' + dateStr + ' à ' + timeStr + ')',
        html: '<div style="font-family:Arial,sans-serif;color:#1e293b;line-height:1.6;max-width:520px;">' +
          '<p>Un rendez-vous vient d\'etre planifie :</p>' +
          '<ul>' +
            '<li><strong>Prospect :</strong> ' + (lead.name || 'Inconnu') + '</li>' +
            '<li><strong>Date :</strong> ' + dateStr + ' a ' + timeStr + '</li>' +
            '<li><strong>Duree :</strong> ' + durationMinutes + ' minutes</li>' +
            (location ? '<li><strong>Lieu :</strong> ' + location + '</li>' : '') +
            (notes ? '<li><strong>Notes :</strong> ' + notes + '</li>' : '') +
          '</ul>' +
          '<p>Le fichier .ics est joint a cet email — ouvrez-le pour l\'ajouter directement a votre calendrier.</p>' +
        '</div>',
        attachments: [{ filename: 'rdv-' + leadId + '.ics', content: icsContent, contentType: 'text/calendar' }]
      });
      console.log('Email RDV confirme envoye a:', agencyConfigForRdv.notification_email);
    } catch(mailErr) {
      console.error('Email RDV erreur:', mailErr.message);
    }
  } else {
    console.log('Email RDV IGNORE — mailTransporter:', !!mailTransporter, '| notification_email configure:', !!(agencyConfigForRdv && agencyConfigForRdv.notification_email));
  }

  if (agencyConfigForRdv && agencyConfigForRdv.telegram_chat_id) {
    await sendRdvTelegram(agencyConfigForRdv.telegram_chat_id, lead, dateStr, timeStr, durationMinutes, location, notes);
  } else {
    console.log('Telegram RDV IGNORE — aucun telegram_chat_id configure pour ce client.');
  }

  return { icsContent: icsContent, lead: lead, dateStr: dateStr, timeStr: timeStr };
}
...
app.post('/api/lead/:id/rdv', async function(req, res) {
  ...
  try {
    ...
    var result = await finalizeRdv(leadId, startAt, durationMinutes, title, location, notes);
...
app.get('/api/rdv/upcoming', async function(req, res) {
...
app.get('/api/lead/:sessionId/summary', async function(req, res) {
  ...
  try {
    ...
    var prompt =
      ...
      'Reste concis (5 a 8 phrases maximum). Redige ce resume dans la meme langue que celle utilisee dans la conversation ci-dessus.';
...
app.post('/api/config', async function(req, res) {
  ...
  try {
    var payload = {
      ...
      rdv_days:               req.body.rdvDays        || undefined, // ex: "1,2,3,4,5" (1=lundi ... 7=dimanche)
      rdv_start:              req.body.rdvStart       || undefined, // ex: "09:00"
      rdv_end:                req.body.rdvEnd         || undefined, // ex: "17:00"
      rdv_slot_minutes:       req.body.rdvSlotMinutes ? parseInt(req.body.rdvSlotMinutes, 10) : undefined,
      default_location:       req.body.defaultLocation || undefined,
      telegram_chat_id:       req.body.telegramChatId || undefined,
      rdv_auto_confirm:       typeof req.body.rdvAutoConfirm !== 'undefined' ? !!req.body.rdvAutoConfirm : undefined,
...
app.post('/api/assistant/:clientId', async function(req, res) {
  ...
  try {
    ...
    var context =
      'Tu es l\'assistant analytique de ' + (configR && configR.agency_name || 'RELAY') + '.\n' +
      'Donnees actuelles:\n' +
      '- Total leads: ' + total + '\n' +
      '- Leads chauds: ' + hot + '\n' +
      '- Leads warm: ' + warm + '\n' +
      ...
      'Reponds de facon concise et professionnelle, dans la meme langue que celle utilisee dans la question posee, sans balises DATA.';
